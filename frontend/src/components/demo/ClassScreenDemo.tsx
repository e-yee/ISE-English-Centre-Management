...
interface ClassScreenDemoProps {
  classId?: string; // Optional prop to specify which class to display
  className?: string;
}

// Internal component that uses the sidebar context
const ClassScreenDemoContent: React.FC<ClassScreenDemoProps> = ({ classId = 'CL001', className }) => {
...
const ClassScreenDemo: React.FC<ClassScreenDemoProps> = ({ classId, className }) => {
  return (
    <SidebarProvider defaultOpen={true}>
      <ClassScreenDemoContent classId={classId} className={className} />
    </SidebarProvider>
  );
};

export default ClassScreenDemo;

...
class Issue(Base):
    __tablename__ = 'issue'
    __table_args__ = (
        CheckConstraint("((issue_type = 'Student Behavior') AND (student_id IS NOT NULL) AND (room_id IS NULL)) OR ((issue_type = 'Technical') AND (student_id IS NULL) AND (room_id IS NOT NULL))", name='CHK_issue_student_room'),
    ...
    issue_description: Mapped[str] = mapped_column(VARCHAR(200))
    status: Mapped[str] = mapped_column(String(20), server_default=text("In Progress"))
    reported_date: Mapped[datetime.date] = mapped_column(Date, server_default=text('(curdate())'))
    student_id: Mapped[Optional[str]] = mapped_column(String(10))
    room_id: Mapped[Optional[str]] = mapped_column(String(10))

    room: Mapped[Optional['Room']] = relationship('Room', back_populates='issue')
    student: Mapped[Optional['Student']] = relationship('Student', back_populates='issue')
    teacher: Mapped['Employee'] = relationship('Employee', back_populates='issue')
...
class Evaluation(Base):
    ...
    __table_args__ = (
        CheckConstraint("assessment_type IN ('Quiz 1', 'Quiz 2', 'Quiz 3', 'Quiz 4', 'Writing Project 1', 'Writing Project 2', 'Reading Assessment 1', 'Reading Assessment 2')", name='CHK_evaluation_type'),
        ForeignKeyConstraint(['class_id', 'class_date'], ['class.id', 'class.created_date'], name='FK_evaluation_class'),
    ...
    comment: Mapped[str] = mapped_column(VARCHAR(2000))
    enrolment_id: Mapped[str] = mapped_column(String(10))
    evaluation_date: Mapped[datetime.date] = mapped_column(Date)

    class_: Mapped['Class'] = relationship('Class', back_populates='evaluation')
    enrolment: Mapped['Enrolment'] = relationship('Enrolment', back_populates='evaluation')
    student: Mapped['Student'] = relationship('Student', back_populates='evaluation')
    teacher: Mapped['Employee'] = relationship('Employee', back_populates='evaluation')
...
class PDF(FPDF):
    ...
    def chapter_title(self, title):
        ...
        # Use effective page width instead of hard-coded 190
        epw = self.w - self.l_margin - self.r_margin
        self.cell(epw, 10, title, 0, 1, "L", fill=True)
        self.ln(5)
    ...
    def create_table(self, headers, data, col_widths=None, header_fill=(230, 240, 255), line_height=8):
        ...
        widths = self._compute_column_widths(headers, col_widths)

        # Table header with wrap and page break handling
        y_start = self.get_y()
        x_start = self.get_x()
        row_h = self._row_height(headers, widths, line_height)
...
def generate_report(data, output_path, logo_path=None):
    ...
    # --- Top two-column section (responsive) ---
    ...
    y_top = pdf.get_y()
    x_left = pdf.get_x()
    pdf.multi_cell(col_w, 8, f"ID: {data.get('student_id','')}\nName: {data.get('student_name','')}", border=0)
    y_after_left = pdf.get_y()

    pdf.set_xy(pdf.l_margin + col_w, y_top)
    pdf.multi_cell(col_w, 8, f"ID: {data.get('teacher_id','')}\nName: {data.get('teacher_name','')}", border=0)
